...
class RainAudioEffect(AudioReactiveEffect):
    ...
    def config_updated(self, config):
        ...
        self.drop_animations = [np.load(os.path.join(os.path.dirname(__file__), "effectlets/droplet_0.npy")),
                                np.load(os.path.join(os.path.dirname(
                                    __file__), "effectlets/droplet_1.npy")),
                                np.load(os.path.join(os.path.dirname(__file__), "effectlets/droplet_2.npy"))]

        self.n_frames, self.frame_width = np.shape(
            self.drop_animations[self._config["raindrop_animation"]])
    ...
    def get_drops(self):
        ...
        for index in drop_indices:
            coloured_frame = [self.drop_animations[self._config["raindrop_animation"]]
                              [self.drop_frames[index]] * self.drop_colours[colour, index] for colour in range(3)]
